...
using Random
    ...
    @testset "Robustbase.jl" begin
        ...
        @testset "Covariance" begin
            ...
            ## Test partitions
            ##  In the following three test cases add tolerance, since it can 
            ##  give difference on other systems. There is no difference on Windows.
            Random.seed!(1234)
            dd = randn(1000, 3)
            mcd=CovMcd(); 
            fit!(mcd, dd);
            @test isapprox(location(mcd), [-0.03709418160384368, -0.04345820016108941, 0.029120962594079838], atol=0.1)
            @test isapprox(covariance(mcd), [1.0559351214910597 -0.014130180358259194 0.010069181114157588; -0.014130180358259194 0.958846597311482 0.052548818362356975; 0.010069181114157588 0.052548818362356975 0.9874841598532943], atol=0.1)
            ...
            mcd=CovMcd(); 
            fit!(mcd, dd);
            @test isapprox(location(mcd), [-0.06265491281120152, -0.009394214619415213, 0.006172716398845805], atol=0.01)
            @test isapprox(covariance(mcd), [1.1250810157701676 -0.034212982553371796 0.08682362639710196; -0.034212982553371796 0.9500897070368451 0.052661546065787525; 0.08682362639710196 0.052661546065787525 1.0145558132234926], atol=0.1)
            ...
            mcd=CovMcd(); 
            fit!(mcd, dd);
            @test isapprox(location(mcd), [-0.022150791228884206, -0.0034405597512334085, -0.002805846879562287], atol=0.01)
            @test isapprox(covariance(mcd), [1.0343487388080232 -0.011519769324182357 -0.0073441160310625045; -0.011519769324182357 1.0387703779359443 0.011906631852494534; -0.0073441160310625045 0.011906631852494534 0.9521863436685187], atol=0.1)
